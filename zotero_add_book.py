...
def extract_book_info(decode_html):
    ...
    if re.findall("<li><b>ISBN-13:</b> (.*)</li>", decode_html):
        ...
        page_result = re.findall(
            "<li><b>(?:Hardcover|Paperback):</b> ([0-9]+) pages</li>", decode_html
        )
        if page_result:
            numPages = page_result[0]
...
def set_pdf_name(book_info):
    ...
    if len(author_name_parts) > 1:
        if len(author_name_parts) > 2:
            ...
            pdf_author = "{}_{}".format(
                author_name_parts[0][1], author_name_parts[1][1]
            )
    else:
        pdf_author = author_name_parts[0][1]
    if book_info["publish_year"]:
        pdf_name = "{}_{}_{}.pdf".format(
            pdf_author, book_info["publish_year"], pdf_title
        )
    else:
...
def copy_pdf_to_dropbox(pdf_name):
    pdf_path = (
        os.popen('rm_invalid_chars "`newest {}/*.pdf`"'.format(book_dir)).read().strip()
    )
    target_pdf_path = "{}/{}".format(
        dropbox_zotero_storage_dir, pdf_name.replace(" ", "\\ ")
    )
